...
import { purchaseExtraBmPackage } from "@/lib/billing/purchase-extra-bm-package";
...
export default function ExtraBmPackageModal({ open, onClose }: Props) {
  const [addBmPackages, setAddBmPackages] = useState(1);
  ...
  const basePrice = ADDONS.existingProjectBusinessManager.priceMonthly;
  const adPrice = ADDONS.extraProjectAdAccount.priceMonthly;

  const total =
    addBmPackages * basePrice + addNestedAds * adPrice;

  const canBuy =
    !buying && (addBmPackages > 0 || addNestedAds > 0);

  async function handleBuy() {
    try {
      setBuying(true);
      setError("");
      await purchaseExtraBmPackage({
        packages: addBmPackages,
    ...
    >
      ...
      >
        ...
        <h2 className="text-xl font-semibold mb-2">
          Extra Business Manager Package
        </h2>
        <p className="text-sm text-zinc-400 leading-relaxed mb-5">
          Base package: ${basePrice}/month. Includes 1 Business Manager + 1 Ad Account.
        </p>

        <CounterRow
          label="Extra BM Packages"
          help={`$${basePrice}/mo each — adds 1 BM, 1 Ad Account`}
          value={addBmPackages}
          onChange={setAddBmPackages}
          min={0}
        />
        ...
        <CounterRow
          label="Extra Ad Account for this BM"
        ...
        <p className="text-[11px] text-zinc-500 mt-2">
          Tip: To add another Business Manager, purchase a separate Extra Business Manager Package.
        </p>
