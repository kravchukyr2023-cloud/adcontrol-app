...
/**
 * Bulk UPSERT adset-level daily insights into meta_adset_insights.
 *
 * Layer: DB only.
 *
 * ON CONFLICT (user_id, meta_adset_id, date) DO UPDATE — matches
 * meta_adset_insights_resource_date_unique.
 */
export async function upsertAdsetInsights(params: {
  userId: string;
  ...
  for (let i = 0; i < batches.length; i++) {
    ...
    const rows = batch.map((r) => ({
      ...
      meta_adset_id_fk: fkMap.get(r.resource_id) ?? null,
      meta_adset_id: r.resource_id,
      date: r.date,
      spend: r.spend,
    ...
    const { error } = await sb
      ...
      .upsert(rows, { onConflict: "user_id,meta_adset_id,date" });
