...
/**
 * Shared buyer-voice invariants (Sprint 6.5 Stage 2). Reused by the entity
 * polish prompt in polish-entities.ts so tone/rules don't drift between the
 * two AI surfaces — one edit, both layers move in sync.
 */
export const BUYER_VOICE_INVARIANTS = `ЖОРСТКІ ІНВАРІАНТИ (порушення = баг):
1. Усі числа, відсотки, суми, назви кампаній/адсетів/оголошень бери ТОЧНО з наданих даних. Не округлюй інакше, не змінюй, не додавай, не прибирай числа.
2. Ніяких чисел, яких немає у вхідних даних. Якщо число потрібне для сенсу, а його немає — переформулюй без нього.
3. Не давай загальних порад "з голови" (типу "оптимізуй витрати", "перевір креативи", "проаналізуй аудиторію"), якщо їх немає у наданих даних. Виходити за межі даних заборонено.
4. Не перетворюй конкретику на абстракцію: "звільни 70% бюджету кампанії X" ≠ "оптимізуй витрати". Директиви залишаються директивами з цифрами.
...
Додатковий інваріант для цього промту:
- Якщо в recommended_action перелічено задачі (напр. "Задача: 1. … 2. … 3. …") — у полі action мають лишитися ВСІ ці задачі, у ТОМУ Ж порядку, з тими ж числами й назвами. Не пропускай кроки, не міняй їх послідовність, не об'єднуй два кроки в один.
